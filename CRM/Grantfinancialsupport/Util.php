...
class CRM_Grantfinancialsupport_Util {
  ...
  public static function deleteGrantFinancialEntries($grantID) {
    $sql = "SELECT
      GROUP_CONCAT(DISTINCT(fi.id)) AS financial_item,
    ...
    while($dao->fetch()) {
      if ($dao->trxn) {
        $ftIDs = explode(',', $dao->trxn);
        foreach ($ftIDs as $id) {
          civicrm_api3('FinancialTrxn', 'delete', ['id' => $id]);
        }
      }
      if ($dao->financial_item) {
        $fiIDs = explode(',', $dao->financial_item);
        foreach ($fiIDs as $id) {
          civicrm_api3('FinancialItem', 'delete', ['id' => $id]);
        }
      }
      if (CRM_Core_DAO::singleValueQuery("SHOW TABLES LIKE 'civicrm_payment'") == 'civicrm_payment' && $dao->trxn) {
        CRM_Core_DAO::executeQuery("DELETE FROM civicrm_payment WHERE financial_trxn_id IN ($dao->trxn)");
      }
      if ($dao->entity_trxn_2) {
        $eftIDs = explode(',', $dao->entity_trxn_2);
        foreach ($eftIDs as $id) {
